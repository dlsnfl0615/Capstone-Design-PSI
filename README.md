...
**파티셔닝 (Partitioning):**
계산 복잡도를 낮추기 위해 각 버킷의 아이템들을 alpha개의 파티션으로 분할하여 관리합니다. 각 파티션은 독립적인 다항식을 구성하게 됩니다.

**다항식 계수 계산:**
각 파티션에 속한 명제들을 근으로 갖는 다항식의 평문 계수(Coefficient)들을 오버플로우 없이 모듈러 산술을 통해 도출합니다.

**거듭제곱 암호문 생성 및 평가:**
Receiver의 윈도잉 암호문(powers.bin)과 매칭하여 0승부터 최대 차수까지의 모든 암호문을 구성한 뒤, 계수와 곱하여 암호화된 다항식 평가 결과(result.bin)를 최종 합성합니다.
