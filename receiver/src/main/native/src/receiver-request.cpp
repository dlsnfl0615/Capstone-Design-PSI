...
int request() {
    ...
    // 공개키 저장
    ofstream pk_out("storage/public_key.bin", ios::binary);
    public_key.save(pk_out);
    pk_out.close();

    // 비밀키 저장
    ofstream sk_out("storage/secret_key.bin", ios::binary);
    secret_key.save(sk_out);
    sk_out.close();

    // relin 키 저장
    ofstream rk_out("storage/relin_key.bin", ios::binary);
    relin_keys.save(rk_out);
    rk_out.close();

    // 해시 테이블 저장
    ofstream hash_out("storage/receiver_hash.bin", ios::binary);
    ...
    // 윈도잉 결과 저장
    ofstream ofs("storage/powers.bin", ios::binary);
