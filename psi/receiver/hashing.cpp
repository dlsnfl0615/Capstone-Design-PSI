...
void Hashing::locate(const vector<string> data) {
...
vector<uint64_t> Hashing::batching(size_t slot_count) {
    vector<uint64_t> batched_contents(slot_count, 0);

    for (size_t i = 0; i < m; i++) {
        // 뻐꾸기 해싱 결과가 저장된 hash_table[i] 값을 슬롯에 할당함.
        batched_contents[i] = hash_table[i];
    }
    return batched_contents;
}

RestoredData Hashing::restore_original_data(uint64_t packed, int loc) {
