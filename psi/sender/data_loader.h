...
//0330 01:26 추가 : fhe_setup 검증을 위해 입려 데이터를 벡터로 변환하는 함수 추가
// combined 문자열(주민번호13자리 + 질병10자리)을 uint64_t 하나로 패킹
uint64_t pack_combined_to_u64(const std::string& combined);

// sender 데이터를 FHE 입력 벡터로 변환
std::vector<uint64_t> make_sender_input_u64(const std::vector<std::string>& data,
                                            size_t max_items);